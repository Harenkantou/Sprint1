...
public class FrontServlet extends HttpServlet {
    ...
    private void invokeController(URLRoute route, String path, HttpServletRequest req, HttpServletResponse res) 
        ...
        try {
            ...
            handleReturnType(result, req, res);
    ...
    /**
     * Gère le type de retour de la méthode du contrôleur
     * - Si c'est un ModelView: transfère les données et redirige
     * - Si c'est un String: écrit directement dans la réponse
     * - Si c'est void: la méthode a déjà géré la réponse
     */
    private void handleReturnType(Object result, HttpServletRequest req, HttpServletResponse res) 
            throws ServletException, IOException {
        
        if (result instanceof ModelView) {
            ModelView modelView = (ModelView) result;
            
            // Ajouter toutes les données du ModelView comme attributs de la requête
            ...
            if (targetUrl != null && !targetUrl.isEmpty()) {
                ...
                // Si pas d'URL spécifiée, afficher les données en JSON
                displayModelViewData(modelView, req, res);
            }
            
        } else if (result instanceof String) {
            // Retour direct sous forme de texte
            res.setContentType("text/html;charset=UTF-8");
            PrintWriter out = res.getWriter();
            out.write((String) result);
        }
        // Si result est null ou void, on suppose que la méthode a déjà géré la réponse
    }

    /**
     * Affiche les données du ModelView en format HTML (fallback)
     */
    private void displayModelViewData(ModelView modelView, HttpServletRequest req, HttpServletResponse res) 
            throws IOException {
        res.setContentType("text/html;charset=UTF-8");
        PrintWriter out = res.getWriter();
        ...
        out.println("<html>");
        out.println("<head>");
        out.println("    <title>ModelView Data</title>");
        out.println("    <style>");
        out.println("        body { font-family: Arial, sans-serif; margin: 20px; }");
        out.println("        .data-item { margin: 10px 0; padding: 10px; background: #f5f5f5; }");
        out.println("        .key { font-weight: bold; color: #333; }");
        out.println("        .value { color: #666; }");
        out.println("    </style>");
        out.println("</head>");
        out.println("<body>");
        out.println("    <h1>Données ModelView</h1>");
        
        if (modelView.getData().isEmpty()) {
            out.println("    <p>Aucune donnée dans le ModelView</p>");
        } else {
            for (Map.Entry<String, Object> entry : modelView.getData().entrySet()) {
                out.println("    <div class='data-item'>");
                out.println("        <span class='key'>" + entry.getKey() + ":</span>");
                out.println("        <span class='value'>" + entry.getValue() + "</span>");
                out.println("    </div>");
            }
        }
