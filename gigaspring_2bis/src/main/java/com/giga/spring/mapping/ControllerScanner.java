...
import com.giga.spring.controller.Controller;
    ...
    private static List<URLRoute> scanClass(String className) {
        ...
        try {
            ...
            try {
                ...
                // Some classes may reference unavailable types (e.g. servlet API)
                // when running a standalone Main. Skip such classes instead of
                // letting the scanner crash with NoClassDefFoundError.
                System.err.println("Skipping class (cannot load): " + className + " - " + e);
                return routes;
            }
            
            // Vérifier si c'est un contrôleur
            if (!Controller.class.isAssignableFrom(clazz) || clazz.equals(Controller.class)) {
                return routes;
            }

            // Ne retenir que les classes explicitement annotées avec @AnnotationClass
            if (!clazz.isAnnotationPresent(AnnotationClass.class)) {
                // skip classes that are controllers by inheritance but not annotated
                return routes;
            }
            ...
            for (Method method : clazz.getDeclaredMethods()) {
                ...
                if (annotation != null) {
                    ...
                    routes.add(route);
                    System.out.println("Route enregistrée: " + urlPattern + " -> " + 
