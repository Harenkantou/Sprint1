// ControllerScanner.java (modifié)
...
import com.giga.spring.annotation.GetUrl; // NEW
import com.giga.spring.annotation.PostUrl; // NEW
    ...
    private static List<URLRoute> scanClass(String className) {
        ...
        try {
            ...
            for (Method method : clazz.getDeclaredMethods()) {
                // Vérifier @GetUrl
                GetUrl getAnnotation = method.getAnnotation(GetUrl.class);
                if (getAnnotation != null) {
                    routes.add(new URLRoute(getAnnotation.value(), controller, method, "GET"));
                }
                
                // Vérifier @PostUrl
                PostUrl postAnnotation = method.getAnnotation(PostUrl.class);
                if (postAnnotation != null) {
                    routes.add(new URLRoute(postAnnotation.value(), controller, method, "POST"));
                }
                
                // Vérifier @URLMapping (rétrocompatibilité)
                URLMapping mappingAnnotation = method.getAnnotation(URLMapping.class);
                if (mappingAnnotation != null && getAnnotation == null && postAnnotation == null) {
                    routes.add(new URLRoute(mappingAnnotation.value(), controller, method, "ANY"));
                }
            }
        } catch (Exception e) {
